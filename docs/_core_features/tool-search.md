...
When a persisted chat's message model has `tool_references` and
`tool_search_blocks` JSON columns, the search exchange is persisted and
restored with the conversation, so discovered tools stay discovered across
process restarts. Without those columns everything still works — the model
just re-searches when a restored conversation needs a deferred tool again.
